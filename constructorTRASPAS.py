...
def base_mana(eleccion, ncartas=17):
    '''
    Calcula la base mana necesaria para
    las cartas elegidas por el algoritmo
    '''
    opts = {'R', 'G', 'W', 'B', 'U'}
    costes = list()
    for carta in eleccion:
        for elem in carta.coste_mana:
            if elem in opts:
                costes.append(elem)
    col1 = [c for c in costes if c == costes[0]]
    col2 = [c for c in costes if c != costes[0]]
    num = round((len(col1)/len(costes))*ncartas)
    return ('#####\nBASE DE MANA\n#####\n'+str(col1[0]) + ' -> '
...
def Main():
    ...
    if eleccion == '-1':
        ...
        # si el archivo cargado es un txt
        if eleccion.split('.')[-1] == 'txt':
            pool = gp.cargar_lista(eleccion)
        else:
            pool = gp.cargar(eleccion)
